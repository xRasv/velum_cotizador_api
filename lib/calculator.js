 ...
 * @param {number} price  - Q per YARD
 * @param {number} profit - %, default 100
 * @param {number} iva    - %, default 12
 * @param {number} extras - Q flat, default 0
 */
export function calculateEnrollable(width, height, price, profit = 100, iva = 12, extras = 0) {
  const fabricCost = ((height + 0.2) / 0.91) * price;
 ...
 * @param {number} width  - meters, > 0
 * @param {number} height - meters, > 0
 * @param {number} price  - Q per METER
 * @param {number} profit - %, default 100
 * @param {number} iva    - %, default 12
 * @param {number} extras - Q flat, default 0
 */
export function calculateDiaYNoche(width, height, price, profit = 100, iva = 12, extras = 0) {
  const fabricYards = ((height + 0.2) / 0.91) * 2;
  const fabricCost = fabricYards * price;
 ...
 * @param {number} extras  - Q flat, default 0
 */
export function calculateTradicional(width, height, price, galeria, profit = 100, iva = 12, extras = 0) {
  ...
  const fabricCost = (2.5 * width * price) / 0.91;
 ...
 * @param {number} [params.price] - not needed for vertical
...
export function calculateProduct(params) {
  ...
  switch (product_type) {
    case 'enrollable':
      return calculateEnrollable(width, height, price, profit, iva, extras);
    case 'dia_y_noche':
      return calculateDiaYNoche(width, height, price, profit, iva, extras);
    case 'tradicional':
      return calculateTradicional(width, height, price, galeria, profit, iva, extras);
    case 'vertical':
